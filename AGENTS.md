...
Core classes: `OOPDI` (entry point, owns the single `MetadataRepository` instance and, if enabled, `metadata.MetadataWarmup`), `Context` (orchestrates instance creation/injection/lifecycle via `InstanceFactory` and `LifecycleProcessor`; owns the `ShutdownStatus` state machine), `GraphValidator` (dry-run startup validation of the reachable bean graph, no instantiation), `ProxyManager` (Byte Buddy proxies via `proxy.ByteBuddyProxyFactory` — generated proxy *classes* are cached globally and shared across containers, per-proxy interception state travels in instance fields via `proxy.ProxiedBeanCarrier`/`ProxyTarget`; REQUEST-scope `ThreadLocal` via `proxy.RequestScopeManager`), `metadata.MetadataRepository`/`ClassMetadata` (single source of truth for a class's primary constructor, field injection points, `@PostConstruct`/`@PreDestroy` methods), `metadata.MetadataMode` (`DISABLED`/`METADATA_ONLY`/`WARMUP_FAIL_FAST`/`WARMUP_LENIENT`, configured via `oopdi.metadata.mode` system property, default `DISABLED`), `metadata.MetadataWarmup`/`WarmupStatus` (background classpath scan that pre-populates the metadata cache when a `WARMUP_*` mode is active), `resolver.DependencyResolutionContext` (public: `getOrCreate` only) / `resolver.InternalResolutionContext` (framework-internal: additionally proxy issuance), `ScopedInstances` (maps `Scope → InstancesState`), `InstancesState` (per-scope instance cache + locks), `ClassesResolver` (delegates classpath scanning to `ClasspathScanner` and filtering to `InjectableFilter`), `Scope` (enum, polymorphic scope selection *and* supplier creation — no switch statements).
...
- Generated proxy classes are shared across containers (`ByteBuddyProxyFactory` static cache); the cache pins bean classes for the JVM lifetime (documented limitation for hot-redeploy environments).
...
- `immediate=true` is only valid for GLOBAL scope; it is invalid for THREAD, LOCAL and REQUEST (misconfiguration fails fast with `CannotInject`).
- Scope state is container-local for every scope — different `OOPDI` instances never share bean instances: GLOBAL/THREAD via the per-container `ScopedInstances`, REQUEST via the per-container `RequestScopeManager` (one instance per container, shared by the interception path in `ProxyManager` and the selection path in `ScopedInstances`; never static); LOCAL trivially (fresh instance per call).
- `OOPDI.validate()` dry-validates the reachable bean graph (no instantiation, no side effects) and aggregates all structural problems into one `CannotInject`; LOCAL beans intentionally have no lifecycle end (call-transient) — no `@PreDestroy` support planned for them.
...
- Fixed: `directConstructionPhase` is save/restore across nested resolutions (not set/reset), so outer chains keep observing direct construction after nested chains finish; outermost completion removes the `ThreadLocal` entry (no pool leak).
- Fixed: `shutdown()` requested before first use is remembered (`OOPDI.shutdownRequested`) — later `getInstance()` calls fail fast with `ContainerShutdown` instead of serving beans from a shut-down container.
- Fixed: THREAD scope has no supplier-level `ThreadLocal` cache anymore (it pinned beans for pooled-thread lifetimes); per-thread caching lives canonically in `ScopedInstances`, whose thread states are dropped at shutdown end.
...
- Shutdown is state-managed (`ShutdownStatus`: `ACTIVE` → `SHUTTING_DOWN` → `SHUTDOWN`/`FAILED`, single-winner CAS, idempotent): a single guard at the top of `Context.getOrCreate` — the sole funnel for all real-object creation — throws `ContainerShutdown` for anything requested after shutdown began (top-level and nested alike). `shutdown()` destroys best-effort in drain-loop passes until no undestroyed instances remain (in-flight chains cannot be aborted and are picked up by later passes), aggregating `@PreDestroy` failures as suppressed exceptions. Failed post-processing (field injection/`@PostConstruct`) compensates via `InstancesState.remove`, so no half-initialized bean stays cached. REQUEST-scoped beans are destroyed at request-chain end (`proxy.RequestScopeManager` intercept `finally`, reverse creation order, same best-effort aggregation; wired via a destroyer `Consumer` set by `Context`); LOCAL beans stay unaffected (call-transient, never cached). Proxy issuance (`getOrCreateProxy`) is framework-internal only (`resolver.InternalResolutionContext`; removed from the public `DependencyResolutionContext`).
...
- Error messages are built with `String.formatted()`, never with `+` concatenation (stable standard API — not JEP 430/431 string templates, so no `--enable-preview` needed).
