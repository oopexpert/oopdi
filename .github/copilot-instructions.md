...
- Remaining known gap (fix planned, Phase B/C of session plan): `ClassesResolver` classpath scan uses `Class.forName` with eager initialization (runs static initializers even for classes later filtered out); `Context.processField` calls `field.setAccessible(true)` unconditionally before checking which inject annotation is present.
