...
- `OOPDI.validate()` dry-validates the bean graph reachable from the root class without creating a single instance (opt-in boot check): `GraphValidator` walks constructor params, field injection points (`MetadataRepository`), `@InjectSet` hints (`ClassesResolver`), `@PostConstruct` params and lifecycle cardinalities, reusing the runtime checks (`InstanceFactory.validateEligible`, shared `VariableDependencyResolver.requireVariableValue` helper so messages stay identical) with the container's active profiles; cycles are reported with paths via a visiting-set, all problems aggregated into one `CannotInject` (causes suppressed). No special cases for REQUEST/LOCAL (nothing is instantiated, so no chains or throwaway instances are needed). Tests: `TestStartupValidation` (fixtures `ClassBrokenGraphRoot`, `ClassBrokenCycleA/B`).
