...
- `OOPDI.validate()` dry-validates the bean graph reachable from the root class without creating a single instance (opt-in boot check): `GraphValidator` walks constructor params, field injection points (`MetadataRepository`), `@InjectSet` hints **including element subgraphs** (`ClassesResolver`), `@PostConstruct` params, `immediate` configuration (`InstanceFactory.checkImmediateInstantiationConfiguration`, reused) and lifecycle cardinalities (via shared `LifecycleProcessor.findPostConstructMethod/findPreDestroyMethod`, so messages stay identical), reusing the runtime checks (`InstanceFactory.validateEligible`, shared `VariableDependencyResolver.requireVariableValue` helper **plus a trial parse** with a default `TypeParserRegistry`, so unparsable values are caught too) with the container's active profiles; cycles are reported with paths **only for constructor-edge loops** (field/set/lifecycle loops resolve at runtime via cached instances and stay silent — tracked per edge in a `Step` record, mirroring the `UnderConstruction` protocol that lives exclusively in the construction phase); all problems aggregated into one `CannotInject` (causes suppressed). No special cases for REQUEST/LOCAL (nothing is instantiated, so no chains or throwaway instances are needed). Tests: `TestStartupValidation` (fixtures `ClassBrokenGraphRoot`, `ClassBrokenCycleA/B`, `ClassFieldCycleA/B` (must stay silent), `ClassBrokenSetRoot/Base/Impl`, `ClassBrokenFormatVar`).
- `directConstructionPhase` is save/restore across nested resolutions (not set/reset): an outer chain resolving its 2nd+ constructor parameter after a nested chain finished still observes direct construction, so nested field dependencies resolve as real objects instead of proxies; outermost completion calls `remove()` (plain `ThreadLocal`, no `withInitial` — no pool entry lingers). Test: `TestResolverAndSet.testNestedFieldInjectionResolvesRealObjectNotProxy` (fixtures `ClassNestedOuter/DepA/DepB`; fails on old code with a ByteBuddy proxy where the real object belongs).
- `shutdown()` requested before first use is remembered (`OOPDI.shutdownRequested`, volatile; `shutdown()` is now `synchronized` like `getContext()`): later `getInstance()` calls fail fast with `ContainerShutdown` instead of serving beans from a shut-down container; `getShutdownStatus()` without a `Context` reports `SHUTDOWN`. Test: `TestShutdownLifecycle.testShutdownBeforeFirstUseIsNotSilentlyLost`.
- THREAD scope has no supplier-level `ThreadLocal` cache anymore (it pinned beans for pooled-thread lifetimes, unreachable for any shutdown hook); per-thread caching lives canonically in `ScopedInstances.threadInstanceMaps`, dropped wholesale at shutdown end via `clearThreadStates` (global state intentionally stays readable). Test: `TestScopedInstances`.
- Failure taxonomy: `DestructionFailed` (shutdown and request-end aggregation, replacing generic `RuntimeException`), `ClasspathScanFailed` (infrastructure scan failures vs. empty/ambiguous filter results), `MultiplePreDestroyMethods` (mirroring `MultiplePostConstructMethods`); eligibility/config problems are `CannotInject` (immediate misconfiguration — message corrected to GLOBAL-only per `Scope`/tests — abstract/non-`@Injectable`/missing-constructor, plus an explicit `Scope.of` guard instead of NPE); unrecognized `MetadataMode` values throw `IllegalArgumentException`. Tests: `TestShutdownLifecycle`/`TestRequestDestruction` assert `DestructionFailed`, `TestLifecycleHooks.testMultiplePreDestroyMethodsThrowDedicatedType` (fixture `ClassMultiplePreDestroyChild`), `TestScopeBehavior.testScopeOfNonInjectableClassFailsDescriptively`.
- Best-effort boundaries (shutdown drain, request-end destruction, warmup scan and per-candidate inspection) catch `Throwable`, documented at each site: an `Error` must neither abort remaining destructions nor die silently with a stale `RUNNING` status.
- All user-facing messages use `String.formatted()`, never `+` concatenation (texts unchanged; stable API, explicitly not JEP 430/431 preview templates, so no `--enable-preview`); German messages unified to English.
- `ClassesResolver.determineRelevantClass` fills `relevantClassCache` atomically via `computeIfAbsent` (previously check-then-act, so racing first-time resolutions each ran a full classpath scan). Mapping-function failures propagate without recording, preserving retry-on-next-call. Test: `TestClassesResolver` (blocking test scanner proves a single shared scan).
- `@InjectVariable optional=true` with a missing key on a primitive field throws a descriptive `CannotInject` (naming the key, advising `defaultValue`/boxed type) instead of failing opaquely later in reflective `Field.set`. Test: `TestVariableInjection.testInjectVariableOptionalPrimitiveFailsDescriptively` (fixture `ClassOptionalPrimitiveVar`).
...
| `ClassBrokenGraphRoot` + `ClassBrokenCycleA`/`ClassBrokenCycleB` | GLOBAL | Deliberately unwirable graph (cycle + non-bean field + missing variable); never instantiated, only dry-validated |
