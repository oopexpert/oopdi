...
Key classes:
- `OOPDI` — entry point; holds a single `Context` (lazy, synchronized); owns the single `MetadataRepository` instance, created eagerly and passed to both `ProxyManager` and `Context`; starts `metadata.MetadataWarmup` when `MetadataMode.isWarmupEnabled()`; exposes `getWarmupStatus()`
